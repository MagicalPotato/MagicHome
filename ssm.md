...
你要么就啥也不干不try-catch,要么就捕获并重新抛出.很多时候我们在代码的service中直接用某个dao调用查库的方法实际上不能称为事务,因为事务涉及到很
多步骤,你单纯的一个调用就不存在什么事务,比如下面这个在某个service中写的updateMethod()你就可以配成一个事务,当第三步骤发生了异常,那么前两步骤
也不会执行.
@Transactional(rollbackFor=Exception.class)   //当然你肯定要在这个方法上加事务注解,或者你就用上面那种配置文件进行配置也行
public updateMethod() {                        //这个注解也可以直接加载service类上,那么类中的所有方法都会被当成事务方法,但是很多情况下
    Dao1.save(Person1);                      //大多公司的service写的都是逻辑.事务的这种东西在金融或者银行系统中应该比较常见.
    Dao1.save(Person2);                    
    Dao1.save(Person2);//假如这句发生了错误,前面那两步会被回退.
    Dao1.save(Person2);
}
