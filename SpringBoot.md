...
##### 抽取maven的父项目
- maven的父亲项目主要是用来做版本管理的,就是说你当前这个项目有很多子模块,每个模块都有一些依赖,但是不同的模块用相同的依赖时版本必须统一,不能是A模块用
的一个版本,B又用了另一个,这样回产生一堆问题.所以统一定义一个父模块来管理版本,其他模块都依赖父模块.
- 我们新建项目初始化的时候选择springbootinitializer,其实就是预先给我们导入boot的jar包,如果只是创建单纯的maven项目,这些jar包就需要我们自己去配.
- 在idea打开的工程中选中project文件夹右键,new-module_直接选择maven;相当于是重新创建了一个模块,只不过这个模块是用来当做父模块的
- 抽取模块的关键在于被抽取的东西的依赖要全部移到新模块中,然后老模块删掉原有抽完报错的信息,最后老模块引入新抽模块的定位坐标.
...
了三个子工程,还是三个单独的工程,最终都是为了部署分布的服务,只要目的能达到,至于过程怎么弄的也不必太在意,只是要记住这种file-new module的方式**
...
直接通过页面访问eureka, h ttp://localhost:8761/eureka/, eureka是有界面的,可以在界面看到注册了哪些东西.
4. 配置生产者和消费者也是一样,在各自模块的配置文件中配置自己的信息和注册中心的信息.  生产者和消费者都可以配置多个,同一个应用你可以修改配置文件中
的端口来产生多个应用.比如这会生产者配置中的端口是8001,你将这个应用先打成jar包拿走 ,然后再将配置中的端口改成8002,再打包拿走,这样就可以同时运行
多个生产者服务,而这些服务都会在注册中心显示.
5. 消费者的主程序上要加@EnableDiscoverClient注解,来发现注册中心的生产者.

##### 杂项
1. 改了东西之后按住ctrl+f9(相当于菜单栏上的build-build project)能重新编译,但是这个编译没法立即应用.可以引入springboot的一个devtools模块,
直接在spring官网就能找到,这样就有热部署能力了.
2. 引入springboot的actuator可以对整个工程进行监控,只要访问工程的ip:8080/对应的监控项,那么就能得到工程对应的监控信息.还可以远程关闭应用,但
必须是post请求来关闭,而且你的工程中要配置响应的配置来Enable远程关闭功能.监控项可以自己定制,具体到时候再看.
