...
表示否定,想要匹配的话要放在除开头以外的位置[8^k56]; 而-如果出现在首位以外则会被当成是范围,如果你想匹配一个-,则需要将其放在首位[-A58]
    ...
    表达式（BRE）和扩展的正则表达式（ERE）,基本的正则只能识别这些元字符 ^ $ . [ ] * 其他的字符全部被当成是普通字符,要想当成元字符必须要用\转义,
    而扩展的正则添加了以下字符作为元字符 ( ) { } ? + |  , 所以并不需要转义. 由于linux的大多程序都只支持基本正则,所以类似于{}这种字符只有被转义
    之后才能表达正则中{}的意思.   一般加--regex选项都是为了表明直接使用扩展正则.
```
