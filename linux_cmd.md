...
```
```
Unix系统中通过一个换行符(ASCII 10)来结束一行,而在MS-DOS和windows中则使用回车(ASCII 13)和换行字符序列来终止每个文本行。
    [me@linuxbox ~]$ cat(sort) > foo.txt  //若在cat和sort后使用了重定向,那么就允许我们创建文件. 当回车后,你可以输入文本,输完之后ctrl+d结束.
    [me@linuxbox ~]$ cat -ns foo.txt   // -n显示行号, -s禁止输出很多空行,连续的空行只输出一行
    [me@linuxbox ~]$ sort file1.txt file2.txt file3.txt > final_sorted_list.txt  //将三个文件内容排序后重新输出到新文件,-m只合并不排序.sort
    还有其他一些选项,-n根据字符串中的数字排序,-b对整行进行排序且忽略行首空格(也就是把文件中每一行当做一个整体,拍所有行,而不是对那一行内部的内容进
    行排序),-k(更常用的是--key=a,b)对a字符和b字符之间的字符进行排序,也就是不把整行当做整体,而只是把一行中的一部分当做标准来排序.-f忽略大小写.
    [me@linuxbox ~]$ sort foo.txt | uniq  //uniq能删除相邻的重复行,所以其输出必须是有序文本. -c能统计重复行的次数, -i忽略大小写

cut 程序能从文本行中抽取文本，并把其输出到标准输出。它能够接受多个文件参数或者标准输入。paste或join把cut抽取出来的文本重新组合成一个新文件.
    
comm用来比较文件,比较后屏幕会输出三列文本,第一列是只有A文件有的,第二列是只有B文件有的,第三列是A和B共有的
    [me@linuxbox ~]$ comm -12 file1.txt file2.txt  //-12表示过滤第一和第二列,只展示第三列, 这里的数字只有123三种情况,可自由组合

diff也可以用来比较文件,比comm更常用. 其中-c和-u选项除了展示方式不同外,其他都一样.-u更简洁一点,不用-c或者-u会很乱. patch文件用来将这种差异应用到你
要进行合并的文件上去.相当于svn的那种功能
    [me@linuxbox ~]$ diff -c file1.txt file2.txt
    [me@linuxbox ~]$ diff -u file1.txt file2.txt
    [me@linuxbox ~]$ diff -Naur a.txt b.txt > c.txt  //首先对比a和b文件,并将差异和两个文件的一些基本信息输出到c文件
    [me@linuxbox ~]$ patch < c.txt   //patch从c中读取文本并将两个文件合并.由于c中已有两个文件的基础信息,所以直接patch不需要再额外指定文件信息

tr程序用来查找和替换
    [me@linuxbox ~]$ echo "lowercase letters" | tr a-z A-Z   //获取标准输入并对其进行替换,然后将结果输出到标准输出. tr命令接受两个参数,要被转换
    的字符集和转换后的字符集. 字符集可以有三种形式的表示: 枚举(也就是举例,比如ABC),像a-z这样的字符域(应该谨慎使用,有时候可能被解析成命令),POSIX字符
    类(就是之前看过的那种linux中专用的正则表示方式例如 [:upper:]),a-z的范围可以比A-Z的范围大,比如tr a-z A,会把所有字符都替换成A.
    [me@linuxbox ~]$ tr -d '\r' < dos_file > unix_file  //-d参数删除指定的字符.这个是将doc中的换行删除 , tr --help ,帮助里有所有转义序列

sed是stream editor（流编辑器）的简称,也即是说sed是个编辑器.既然是编辑器,那么进行一些查找替换,重组文件等操作就是理所当然.可以使用正则
    [me@linuxbox ~]$ echo "front" | sed '1s/front/back/g'  //相当于是vim中的查找和替换,1表示只对文件的第一行做处理,如果输入了不存在的行号,则不
    做任何处理, s是模式,/是分隔符,s后跟啥符号啥就被当成分隔符.比如 s_front_back,这时候_就是分隔符.g是全部的意思,如果不加只会替换首次遇到的
    [me@linuxbox ~]$ sed -n '/SUSE/p' a.txt  //打印a中匹配/USER/的行,两个斜线是分隔符,p是模式(打印),由于sed的默认操作就是打印,但是默认操作会
    打印所有的行,但你要是把所有的行都打印了那么匹配的行不就看不出来了,所以用-n来取消默认的打印,而只对满足条件的数据进行打印. 如果是!n,那意思就是匹配
    的行不打印,也就意味着打印不匹配的行.
    sed 's/\([0-9]\{2\}\)\/\([0-9]\{2\}\)\/\([0-9]\{4\}\)$/\3-\1-\2/' a.txt  //把文件中MM/DD/YY格式的时间修改成YY-MM-DD
    sed 's/([0-9]{2})/([0-9]{2})/([0-9]{4})$/\3-\1-\2/' a.txt  //转义前的样子. 其中([0-9]{2})/([0-9]{2})/([0-9]{4})$是一个有逆参照功能的正
    则,原本 [0-9]{2}/[0-9]{2}/[0-9]{4}$就已经能完成匹配的功能了,现在我给每个正则小组加了一个圆括号,这样我们可以在后续通过指定的方式来使用这个子组,
    \3-\1-\2,这就是使用子组的方式,我们用\3的意思就是我在这个位置引用前面出现的正则中的第三个小组,也就相当于[0-9]{4},\1,\2同理.
```
