...
sed是stream editor（流编辑器）的简称,也即是说sed是个编辑器.既然是编辑器,那么进行一些查找替换,重组文件等操作就是理所当然.可以使用正则
    [me@linuxbox ~]$ echo "front" | sed '1s/front/back/'  //相当于是vim中的查找和替换,1表示只对文件的第一行做处理,如果输入了不存在的行号,则不
    做任何处理, s是模式,/是分隔符,分隔符可以是任意的符号,s后跟啥符号啥就被当成分隔符.比如 s_front_back,这时候_就是分隔符.
