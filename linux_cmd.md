...
su 允许你假定为另一个用户的身份，并以这个用户的 ID 启动一个新的shell会话，或者是以这个用户的身份来发布一个命令。sudo 命令允许一个管理员设置一个
叫做/etc/sudoers的配置文件,并在里面定义一些具体命令,特殊用户就可以执行这些命令.这是同一种操作的两种不同展示方式,用哪个取决于系统里有哪个和习惯.
