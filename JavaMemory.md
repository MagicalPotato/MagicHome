...
```
  ...
  //private Condition con2 = lock.newCondition();  //之所以说Condition可以精准唤醒,就在于一个锁可以绑定多个Condition.
  //这样在类中的方法中使用Condition的时候就可选择性地将不同的方法进行组合.比如当前这个例子中我的testAwait方法和testSignal
  //方法中用的是con,我也可以让另外两个方法用con2,在main方法中我新建A和B的线程对象,分别调testAwait和testAwait2,这时候再来
  // 线程C调用了testSignal,由于testSignal用的是con,所以沉睡的A和B只有A会被唤醒.
...
* ReentrantLock具有完全互斥排他效果,即同一时刻只有一个线程在执行lock()方法后面的任务,这样虽然保证了实例变量的线程安全,但是效率很低.于是jdk提供了一种读写锁ReentrantReadWtireLock.读写锁有两个锁,一个read锁一个write锁,当代码被read锁锁了之后,别的代码依然可以执行被read锁过的代码,但是当代码被write锁锁上之后,别的线程就只能等到这个write锁将锁释放才能执行.

* 多线程并不难理解,一个类中的代码只由一个线程来执行效率肯定低下,但是假设我现在有10个线程来执行这一个类中的代码,那么效率将大大提高.当然,要想让多个线程能执行你的方法,你的类就必须集成Thread或者实现Runnable.所以你的类名变成了xxxThread.....当然,当你在使用多线程的时候,问题也出现了,假如我的类中的一些代码因为某种原因同时刻只能被独立地执行不能进行共享,这时候就是问题的根源.所以才出现了锁这种概念,当类中的一些敏感代码被执行的时候我要告诉别的线程你们不能执行,必须等我执行完才能继续执行.锁就是专门干这个事情的. 而需要在多线程环境下执行的代码,势必会复写线程的run方法,一切的逻辑都在run方法中实现,而能把锁恰到好处地用到你的代码逻辑中就成了一种能力. 当你把使用类中的代码写好之后,你会交给线程去执行,可以是一个线程,也可以是多个线程,你可以一个一个的新建线程,也可以用一循环建一个线程数组然后一次性启一大堆线程,如果你的锁加的没问题,代码逻辑设计的也没问题,那么你的多线程就成功运行了. 有时候你想在某个时间点或者一段时间后去执行某个类中的一些代码,然后你用了个定时器,定时去启动一个线程让它帮你执行你的类中的代码,这样你变得轻松了不少.  千万不要被类名称中的thread给迷惑了,假如有各类叫public class ParseLogThread{},乍一看好牛逼的一个类啊,还是个线程,其实这种乍一看的理解是错误的,这个ParseLogThread本就是一个很普通的类,名字本来也应该叫ParseLog,之所以要加个Thread只不过是为了告诉你这个类可以被多个线程执行,仅此而已.其他的使用方式其实和你使用一个普通的类没什么区别.假如这个ParseLogThread类中除了覆写的run方法外还有别的方法,其中有个方法你恰好可以用到,不用犹豫,你就new一个ParseLogThread然后调用就是了,跟调用一个util工具类没什么区别.
