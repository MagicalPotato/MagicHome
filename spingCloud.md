...
相应的注解(类似于dao层+注解完成sql查询的方式),然后在新的Feign消费者类中直接引入公共接口包中的service接口,用接口调生产者即可.feign和Ribbon都是
用来做负载均衡的,只不过feign的调用方式更加简洁,用接口+注解,代码里没有真实的url,调用的时候感觉就像是在自己调自己.而template则会有明显的url调用.
